...
basic.showLeds(`
    ...
    . # # # .
    `)
// 인간이란 본디 자신이 만든 코드라도 주석 처리를 하지 않으면 못 알아보는 법  근데 이건 내 의견도 들어봐야 하지 않음...? 내가 이 코드 짰는데 어떻게 보면  내가 이 코드를 마음으로 낳은 건데  어머니 오셨어요? 네~ 저 코드에요 기억나시죠?  하면서 통설명을 해줘야 하는 거 아님?  아 이래서 주석 처리가 중요함  내가 마음으로 낳은 자식들 다 패륜함.... 폐륜이란 말이 좀 그렇긴 하다만은..... 다시 주석 넣기 글렀다  하여간 나약해 빠짐....  근데 생각해보면 이게 또 너무하다는 생각이 들긴 함  애초에 만드는 것부터 뇌빼고 만들었는데  의지박약이다 근면성실하게 살자
// 
